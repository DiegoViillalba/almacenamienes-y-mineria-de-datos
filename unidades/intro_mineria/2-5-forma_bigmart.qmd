...
Este caso aplica los conceptos presentados en [Medidas de forma](2-5-forma.qmd). Allí se distinguen asimetría, curtosis, multimodalidad y gráficos Q–Q.
:::
## Introducción
...
**Definición operativa.** La forma describe asimetría, colas y estructura modal después de considerar el centro y la escala de la distribución [@joanes1998skewness].
:::

## Preparación del entorno

Trabajaremos con el siguiente [dataset BigMart](../../data/bigmart_sales.csv), usando Python, pandas, NumPy, SciPy y Plotly.
...
fig.add_trace(go.Histogram(
    x=log_sales, nbinsx=40, histnorm="probability density",
    opacity=0.55, name="Histograma"
))
fig.add_trace(go.Scatter(
    x=xs_log, y=kde_log(xs_log), mode="lines",
    line=dict(width=3), name="Densidad KDE"
))
fig.update_layout(
    title="Histograma y densidad de log(1 + Item_Outlet_Sales)",
    xaxis_title="log(1 + Item_Outlet_Sales)", yaxis_title="Densidad",
    barmode="overlay"
)
fig.show()
```
...
```{python}
def qq_plotly(x, titulo):
    (teoricos, observados), (pendiente, intercepto, _) = stats.probplot(x, dist="norm")
    linea_x = np.array([teoricos.min(), teoricos.max()])
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=teoricos, y=observados, mode="markers", name="Cuantiles"))
    fig.add_trace(go.Scatter(
        x=linea_x, y=intercepto + pendiente * linea_x,
        mode="lines", name="Referencia normal", line=dict(dash="dash")
    ))
    fig.update_layout(title=titulo, xaxis_title="Cuantiles teóricos",
                      yaxis_title="Cuantiles observados")
    return fig

qq_plotly(sales, "Gráfico Q-Q de Item_Outlet_Sales").show()
```

```{python}
qq_plotly(log_sales, "Gráfico Q-Q de log(1 + Item_Outlet_Sales)").show()
```

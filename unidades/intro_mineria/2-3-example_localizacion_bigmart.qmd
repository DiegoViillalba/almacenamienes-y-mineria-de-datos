...
Este capítulo aplica los conceptos desarrollados en [Medidas de localización](2-3-localizacion.qmd). Conviene revisar primero las diferencias entre media, mediana, moda y cuantiles.
:::
...
**Definición operativa.** En esta práctica, una medida de localización resume un valor típico o una posición relevante de una variable numérica; ninguna medida se interpreta sin observar también su distribución [@tukey1977eda; @hyndman1996quantiles].
:::

## Preparación del entorno

Trabajaremos con el siguiente [dataset BigMart](../../data/bigmart_sales.csv), usando Python, pandas, NumPy y Plotly.
...
```{python}
outlet_median_plot = outlet_median.rename("Mediana").reset_index()
fig = px.bar(
    outlet_median_plot,
    x="Outlet_Type",
    y="Mediana",
    color="Outlet_Type",
    text_auto=".2f",
    title="Mediana de Item_Outlet_Sales por Outlet_Type"
)
fig.update_layout(showlegend=False, xaxis_title="Tipo de outlet", yaxis_title="Mediana de ventas")
fig.show()
```

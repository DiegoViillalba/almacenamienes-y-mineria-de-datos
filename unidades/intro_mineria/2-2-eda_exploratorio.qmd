...
Sea una variable categórica u ordinal/discreta con valores posibles $v_1, \dots, v_K$ y una muestra $x_1, \dots, x_n$. La **frecuencia absoluta** y la **frecuencia relativa** (proporción) se definen como:
...
La colección $(p_1,\dots,p_K)$ es la **distribución empírica** sobre las categorías (una estimación no paramétrica de la PMF en el caso discreto). Cuando existe un **orden natural** en los valores (por ejemplo, variable discreta numérica u ordinal con orden predefinido), se definen frecuencias acumuladas:
...
### Ejemplo sintético (categórica): barras + acumulada
...
### Ejemplo sintético (discreta): conteos + acumulada por valor
...
Para $n$ observaciones $x_1,\dots,x_n$, la media muestral es:

$$
\bar{x} \;=\; \frac{1}{n}\sum_{i=1}^n x_i.
$$

La presentación incluye esta fórmula y un ejemplo numérico con edades (media $=24$). 

La media tiene una propiedad algebraica fundamental (linealidad o afinidad):

$$
\overline{aX+b} \;=\; a\,\bar{X} + b,
$$

lo que explica por qué es natural en modelos aditivos y por qué se comporta bien bajo cambios de escala y traslación. Sin embargo, su principal limitación en EDA es su **no robustez**: un solo valor extremo puede afectar $\bar{x}$ de forma desproporcionada. Esto se entiende porque la media “promedia” magnitudes y, por tanto, responde directamente al tamaño del outlier.
...
La mediana es el cuantil $0.5$. Si $x_{(1)}\le \dots \le x_{(n)}$ denota el reordenamiento de los datos, entonces:
...
A diferencia de la media, la mediana depende del **orden** y no de la magnitud exacta de valores extremos. Por eso es una medida **robusta**: su valor no cambia mientras los outliers no alteren de manera sustancial el orden central de la muestra. En EDA, esta propiedad es especialmente útil para:
...
La moda es el valor con frecuencia máxima. Para variables discretas o categóricas, puede formalizarse como:

$$
\operatorname{mode}(x)\;=\;\arg\max_{v_k}\; f_k,
$$

donde $f_k$ es la frecuencia absoluta del nivel $v_k$. Es especialmente informativa en nominales (donde media/mediana no aplican) y también aparece en discretas cuando existe un valor dominante (por ejemplo, muchos ceros).

En limpieza, la moda suele revelar codificaciones problemáticas: si “NA”, “No aplica” o “-” resultan ser la categoría más frecuente, es probable que exista un problema de captura o estandarización de faltantes. También puede ser ambigua si existen varias categorías con la misma frecuencia máxima (multimodalidad), lo cual requiere reportarla como conjunto de modos o cambiar de resumen.

## Ejemplo sintético (e-commerce): gasto por compra con cola derecha (media vs mediana)
...
plt.figure()
plt.hist(lat_sym, bins=70, alpha=0.6, label=f"Sistema estable (skew={sk_sym:.2f})")
plt.hist(lat_skew, bins=70, alpha=0.6, label=f"Sistema con cola (skew={sk_skew:.2f})")
plt.title("Asimetría en latencias: distribución simétrica vs sesgada")
...
for name, x in datasets:
    sk, ku = skew_kurt(x)
    plt.hist(x, bins=80, alpha=0.35, label=f"{name} (sk={sk:.2f}, ku={ku:.2f})")

plt.title("Comparación final: asimetría y curtosis")
plt.xlabel("x (escalas comparables)")
plt.ylabel("Frecuencia")
plt.legend()
plt.tight_layout()
plt.show()
